...
// TestExportWithToken_IncludesResponseBodyOn401 verifies that the error
...
func TestExportWithToken_IncludesResponseBodyOn401(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	exp := newRetryExporter(t, srv)
	md := makeMetrics(nil, "test.metric", 1.0)
	err := exp.exportWithToken(context.Background(), md, "token")
	if err == nil {
...
// TestExportWithToken_IncludesResponseBodyOn403 mirrors the 401 case for a
// 403 Forbidden response.
func TestExportWithToken_IncludesResponseBodyOn403(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
// TestExportWithToken_MapsStatusToGRPCCode verifies that a non-retryable 4xx
// response carries a gRPC status matching its HTTP semantics, so the OTLP
// receiver reports the original status (401/403/400) to the sender instead
// of collapsing every permanent failure into a generic 500.
func TestExportWithToken_MapsStatusToGRPCCode(t *testing.T) {
	tests := []struct {
		status   int
		wantCode codes.Code
	}{
...
// TestExportWithToken_NoGRPCStatusOnServerFailure verifies that a transient
// server-side failure (exhausted retries on 5xx) does NOT carry an explicit
// gRPC status, so the receiver falls back to its default retryable code
// instead of treating it as a permanent client error.
func TestExportWithToken_NoGRPCStatusOnServerFailure(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusInternalServerError)
	}))
	defer srv.Close()
