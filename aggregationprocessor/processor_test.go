...
func TestGetTimeBucketDifferentIntervals(t *testing.T) {
	...
	for _, tc := range cases {
		agg := NewMetricAggregator("user.email", tc.interval, zap.NewNop())
		got := agg.getTimeBucket(ts)
...
func TestClaudeCodeCostAggregationByModel(t *testing.T) {
	...
	agg := NewMetricAggregator("user.email", time.Minute, zap.NewNop())
...
// TestMetricMapCapEnforced verifies that the aggregator stops accepting new
// entries once maxMetricEntries is reached, preventing unbounded memory growth
// from high-cardinality clients.
func TestMetricMapCapEnforced(t *testing.T) {
	agg := NewMetricAggregator("user.email", time.Hour, zap.NewNop())

	// Fill the map to the cap using unique emails.
	for i := 0; i < maxMetricEntries; i++ {
		email := fmt.Sprintf("user%d@example.com", i)
		agg.AddMetrics(createTestMetrics(email, "test.metric", 1.0))
	}
...
// TestMetricMapCapEnforcedGauge verifies the cap applies to gauge metrics too.
func TestMetricMapCapEnforcedGauge(t *testing.T) {
	agg := NewMetricAggregator("user.email", time.Hour, zap.NewNop())

	for i := 0; i < maxMetricEntries; i++ {
		email := fmt.Sprintf("user%d@example.com", i)
		agg.AddMetrics(createTestGaugeMetrics(email, "test.gauge", 1.0, time.Now()))
	}
